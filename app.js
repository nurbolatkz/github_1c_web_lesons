...
- не настраиваем production-публикацию.`,
      ...
      {
        title: "Сгенерировать модуль обработчиков через AI",
        text: "Код обработчиков HTTP-сервиса удобно вынести в отдельный общий модуль — обычный файл .bsl, как типовые модули вроде ЗагрузкаКонвертацияВалюты.bsl. Такой файл можно отдать AI-агенту на генерацию, а затем подключить к HTTP-сервису вручную в Конфигураторе.",
        content: [
          { type: "command", label: "Перейти в папку выгрузки конфигурации", code: "cd onec" },
          { type: "command", label: "Запустить Codex", code: "codex" },
          { type: "command", label: "Или запустить Claude Code", code: "claude" },
          {
            type: "snippet",
            label: "Prompt для генерации ОбработкаЗапросовHTTPСервисаAPI.bsl",
            body: `Ты работаешь в выгруженной в файлы конфигурации 1С (папка onec).

Создай новый общий модуль с именем:

ОбработкаЗапросовHTTPСервисаAPI

Файл модуля: ОбработкаЗапросовHTTPСервисаAPI.bsl
(обычный текстовый модуль, как ЗагрузкаКонвертацияВалюты.bsl).
...
- что нужно проверить вручную перед публикацией.`,
          ...
          {
            type: "note",
            text: "Файл модуля создаётся и редактируется как обычный текст, но подключение обработчика к шаблону HTTP-сервиса нужно сделать вручную в Конфигураторе — AI-агент не имеет доступа к конфигурации 1С напрямую.",
          },
        ],
      },
    ],
