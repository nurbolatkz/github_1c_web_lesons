...
- найденные ограничения.`,
  ...
  },
  {
    ...
    subtitle: "Создайте список, карточку и формы справочника «Товары».",
    minutes: "35 мин",
    level: "Средний",
    complete: false,
    need: [
      "React-приложение из урока 12",
      "Рабочая cookie-сессия",
      "CRUD-маршруты товаров в FastAPI",
      "Документация HTTP-сервиса 1С в onec/",
    ],
    goal: "пользователь сможет просматривать, создавать, изменять и помечать товары на удаление через React и FastAPI.",
    steps: [
      {
        title: "Проверить API-контракт",
        text: "Перед созданием интерфейса проверьте маршруты FastAPI и реальные форматы запросов и ответов. Одной авторизации недостаточно: мост должен предоставлять CRUD товаров.",
        code: `Маршруты FastAPI:
...
Поля товара из контракта курса:
...
Уточните:
- формат списка: массив или объект;
- обязательные поля создания;
- возможность изменения code;
- ответы POST и PATCH;
- возвращает ли список помеченные товары.

Если маршрутов ещё нет, сначала реализуйте их в bridge.
Не заменяйте отсутствующий API постоянными mock-данными.`,
      },
      {
        title: "Создать маршруты страниц",
        text: "Разделите список, просмотр, создание и редактирование. Используйте существующую защиту маршрутов из урока 12.",
        code: `Маршруты React:

/products           — список
/products/new       — создание
/products/:id       — карточка
...
Обновление страницы карточки должно
повторно загружать товар по id.`,
      },
      {
        title: "Создать сервис товаров",
        text: "Вынесите запросы в productsApi.js и используйте существующий API-клиент с cookie-сессией.",
        code: `src/services/productsApi.js
...
markProductForDeletion(id)

Правила:

- запросы только к FastAPI;
- credentials: "include";
- существующая CSRF-защита для изменяющих запросов;
- без access token 1С во frontend;
- без вызовов fetch внутри каждой формы;
- HTTP 204 обрабатывается без чтения JSON;
- ошибки HTTP проверяются явно.`,
      },
      {
        title: "Создать список и карточку",
        text: "Список помогает найти и открыть товар. Карточка показывает актуальные данные, полученные с сервера.",
        code: `Список товаров:
...
Помеченный товар:
показать статус «Помечен на удаление».

Если API исключает помеченные товары,
после обновления списка запись исчезнет.

Не добавляйте поиск и пагинацию через
несуществующие параметры API.`,
      },
      {
        title: "Создать общую форму товара",
        text: "Используйте один ProductForm для создания и редактирования. Клиентская проверка помогает пользователю, но не заменяет проверку в 1С.",
        code: `Поля формы:

- Наименование — обязательное;
- Единица измерения — по контракту;
- Код — только если API разрешает его ввод.

Создание:

POST /api/products

Пример тела при соответствующем контракте:
...
- проверить непустое name после trim;
- не отправлять id и серверные поля;
- отправлять только разрешённые поля;
- блокировать повторное сохранение;
- сохранять введённые значения при ошибке;
- показывать подтверждение только после успеха API.`,
      },
      {
        title: "Добавить пометку удаления",
        text: "DELETE означает пометку удаления в 1С, а не физическое удаление товара.",
        code: `Перед действием показать подтверждение:

«Пометить товар "Кабель" на удаление?
Запись останется в 1С.»
...
1. Выполнить DELETE /api/products/{id}.
2. Дождаться успешного ответа.
3. Показать сообщение «Товар помечен на удаление».
4. Повторно загрузить список или карточку.

Если 1С вернула конфликт:
показать ошибку и оставить запись без изменений.

Не добавлять физическое удаление
или снятие пометки без отдельного контракта.`,
      },
      {
        title: "Обработать состояния и ошибки",
        text: "Не показывайте пустой список вместо ошибки и не объявляйте сохранение успешным до ответа сервера.",
        code: `Состояния:

loading — «Загрузка товаров…»
empty   — «Товаров пока нет»
success — список или карточка
error   — сообщение и кнопка «Повторить»

Ошибки:

400/422 — показать ошибки заполнения формы;
401     — использовать общий flow повторного входа;
403     — «Недостаточно прав», не выполнять logout;
404     — «Товар не найден»;
409     — показать безопасное описание конфликта;
502/504 — «1С временно недоступна».

При смене страницы или id:
отменять запрос или игнорировать устаревший ответ.

POST, PATCH и DELETE не повторять автоматически:
при потере ответа операция уже могла выполниться.`,
      },
      {
        title: "Проверить рабочий сценарий",
        text: "Проверьте интерфейс на учебной базе и убедитесь, что изменения действительно появились в 1С.",
        code: `Сценарий:

1. Войти в приложение.
2. Открыть список товаров.
3. Создать учебный товар.
4. Открыть его карточку.
5. Изменить название и единицу измерения.
6. Обновить страницу и проверить сохранённые данные.
7. Отменить пометку удаления в диалоге.
8. Повторить действие и подтвердить пометку.
9. Проверить результат в 1С.
...
- истёкшая сессия;
- ответ 403;
- недоступность FastAPI или 1С.

В SQL FastAPI остаются только users и sessions.`,
      },
      {
        title: "Проверить и зафиксировать изменения",
        text: "Перед коммитом проверьте сборку и diff. Добавляйте только файлы этого урока, не затрагивая чужие изменения.",
        code: `Из папки frontend:
...
git add -p -- frontend
git diff --cached
git commit -m "Add products CRUD interface"

Новые файлы добавляйте отдельно по точным путям.
Не добавляйте .env и секреты.`,
      },
      {
        title: "Prompt для Codex или Claude Code",
        text: "Запускайте AI-агента внутри папки frontend, где находится React-проект.",
        content: [
          ...
          {
            type: "snippet",
            label: "Prompt для CRUD-интерфейса товаров",
            body: `Ты работаешь в папке frontend. Реализуй урок 13: CRUD-интерфейс справочника «Товары».

Сначала изучи существующее приложение, API-клиент, авторизацию и маршрутизацию. Используй их повторно. Не создавай новый Vite-проект.

Изменяй только frontend. Для сверки контрактов разрешено прочитать документацию в соседней папке onec и маршруты bridge, но не изменять их.

Архитектура: React → FastAPI → HTTP-сервис 1С. Товары хранятся только в 1С. React использует серверную cookie-сессию; access token 1С в браузер не передаётся.

Проверь доступность контрактов:
...
Не выдумывай формат списка, поля, параметры фильтрации и пагинацию. Если контракт отсутствует или противоречив, укажи блокер. Не подменяй реальный CRUD mock-данными.

Создай страницы:

/products — список;
/products/new — создание;
/products/:id — просмотр;
/products/:id/edit — редактирование.

Используй существующий ProtectedRoute. Сохрани общий стиль приложения.

Создай productsApi.js поверх существующего API-клиента:
...
Используй credentials: "include" и существующий механизм CSRF. Не отключай CSRF ради работоспособности формы. Если необходимая серверная поддержка отсутствует, укажи это как блокер.

Проверяй HTTP-статусы. Не вызывай response.json() для 204. Не повторяй изменяющие запросы автоматически.

Создай переиспользуемый ProductForm:

name — обязательное, непустое после trim;
unit — согласно контракту;
code — редактируемое только при разрешении API.

Не отправляй серверные поля. Для PATCH передавай изменённые разрешённые поля. Во время сохранения блокируй повторную отправку. При ошибке сохраняй введённые значения.

В списке и карточке отображай code, name, unit и marked_for_deletion согласно ответу API.

После успешного изменения обновляй серверные данные. Не показывай успех до подтверждения API.

Реализуй подтверждение «Пометить на удаление».

DELETE выполняет только пометку удаления. При отмене диалога запрос не отправляется. После успеха повторно загрузи данные. При 409 покажи безопасное сообщение о конфликте.
...
пустой список;
успешная загрузка;
ошибка и повторное чтение;
сохранение;
пометка удаления.

Обработай 400/422, 401, 403, 404, 409 и 502/504. Для 401 используй общий flow авторизации. При 403 не выполняй logout.

Защити страницы от устаревших ответов при переходах между товарами. Используй существующий механизм загрузки либо отмену запросов и проверку актуальности ответа.

Сохрани доступность интерфейса:
подписи полей;
понятные сообщения ошибок;
управление клавиатурой;
корректный фокус в диалоге подтверждения;
читаемый список на небольшом экране.

Проверь сборку и доступные тесты. Добавь проверки формы, обработки ошибок и DELETE с ответом 204 в существующий тестовый стек.

Интеграционные изменения выполняй только в явно обозначенной учебной базе и на тестовых товарах. Если доступа к ней нет, отдельно укажи, что сквозной сценарий не проверен.

Не создавай коммит автоматически. Покажи изменённые файлы и предложи сообщение коммита: Add products CRUD interface.

После работы верни:

созданные и изменённые файлы;
реализованные страницы;
использованные API-контракты;
фактически выполненные проверки;
блокеры и непроверенные сценарии.

Не показывай пароли, cookie, токены или содержимое секретных файлов.`,
          },
          {
            type: "note",
            text: "Технические нюансы: fetch требует явной проверки HTTP-ошибок, ответ 204 не содержит тела, а устаревшие ответы запросов не должны перезаписывать состояние страницы.",
          },
        ],
      },
    ],
  },
  {
    ...
    subtitle: "Создайте список, форму и сохранение документа «Заявка» с табличной частью товаров.",
    minutes: "35 мин",
    level: "Средний",
    complete: false,
    need: [
      "React-приложение с авторизацией",
      "Справочник товаров из урока 13",
      "CRUD-маршруты заявок в FastAPI",
      "Документация HTTP-сервиса 1С в onec/",
    ],
    goal: "пользователь сможет создать заявку, выбрать товары, изменить строки и проверить сохранённый документ в 1С.",
    steps: [
      {
        title: "Проверить контракт документа",
        text: "Изучите реальные маршруты и JSON-контракт. Заявка хранится в 1С; FastAPI передаёт запросы с access token текущего пользователя.",
        code: `Маршруты FastAPI:
...
- product_id
- product_name — для отображения, если возвращает API
- quantity

До реализации уточните:

- кто назначает номер и дату;
- формат даты;
- формат списка заявок;
- правила изменения items;
- поведение помеченного документа.

Не добавляйте поля и маршруты,
которых нет в контракте.`,
      },
      {
        title: "Создать список и маршруты",
        text: "Список показывает сохранённые заявки, а форма объекта — шапку и табличную часть.",
        code: `Маршруты React:

/requests           — список
/requests/new       — создание
/requests/:id       — просмотр
/requests/:id/edit  — редактирование
...
Используйте существующий ProtectedRoute.
При обновлении страницы загружайте документ по id.`,
      },
      {
        title: "Создать форму шапки",
        text: "Используйте общую RequestForm для создания и редактирования документа.",
        code: `Поля:

Номер:
- показывать значение из 1С;
- не генерировать во frontend.

Дата:
- редактировать только если разрешено контрактом;
- сохранять в согласованном формате;
- не менять календарную дату случайным
  преобразованием часового пояса.

Комментарий:
- текстовое поле;
- до 200 символов по модели урока 04.
...
Эти возможности требуют отдельного контракта.`,
      },
      {
        title: "Добавить ReferencePicker",
        text: "Переиспользуемый компонент выбора показывает товары, но сохраняет ссылку на выбранный объект по его id.",
        code: `Компонент:

src/components/ReferencePicker.jsx

Возможный интерфейс компонента:

- value — выбранный id;
- selectedLabel — название;
- onChange — выбранный товар;
- disabled;
- error.

Поведение:

1. Загрузить товары через productsApi.
2. Показать код и наименование.
3. При выборе сохранить product_id.
4. Показать название в строке заявки.

Название товара — не идентификатор.
Изменение названия не должно разрушать ссылку.

Помеченные товары нельзя выбирать заново.
Уже сохранённую ссылку не удалять молча:
показать предупреждение и применить правила API.

Поиск и пагинация — только согласно контракту.
Локальный поиск по одной странице не является
поиском по всему справочнику.`,
      },
      {
        title: "Создать табличную часть",
        text: "Пользователь должен добавлять строки, выбирать товары, менять количество и удалять строки из формы.",
        code: `Колонки:
...
Каждая строка имеет локальный clientRowId.
Он создаётся один раз и используется как React key.

clientRowId не отправляется в API.
product_id не используйте как key:
один товар может встречаться в нескольких строках,
если это допускает контракт.

Удаление строки из формы не удаляет товар
из справочника и применяется к документу
только после сохранения.`,
      },
      {
        title: "Проверить данные перед сохранением",
        text: "Проверка в React помогает исправить форму. Окончательную проверку ссылок, прав и данных выполняет сервер.",
        code: `Правила:

- в заявке минимум одна строка;
- в каждой строке выбран product_id;
- quantity — конечное число больше нуля;
- точность количества — до 3 знаков
  после запятой по модели урока 04;
- пустое значение нельзя подменять нулём;
- лишнюю точность не округлять молча;
- comment не длиннее 200 символов.

Ошибки показывать рядом с полем и строкой.

Во время сохранения:
- блокировать повторную отправку;
- не разрешать менять отправляемую форму.

При ошибке:
- сохранить введённую шапку и строки.`,
      },
      {
        title: "Сохранить шапку и строки",
        text: "Отправляйте документ одним запросом. Успешное сохранение должно означать, что 1С записала шапку и табличную часть целиком.",
        code: `Пример POST /api/requests,
если контракт принимает дату в формате YYYY-MM-DD:

{
  ...
  "items": [
    {
      "product_id": "<id первого товара>",
      "quantity": 2
    },
    {
      "product_id": "<id второго товара>",
      "quantity": 1.5
    }
  ]
}
...
- clientRowId;
- product_name;
- автоматически назначаемый номер;
- другие поля только для чтения.

Для PATCH заранее подтвердите правило items:

Рекомендуемый контракт:
- items отсутствует — строки не изменяются;
- items передан — полностью заменяет табличную часть;
- items: [] — ошибка, если пустая заявка запрещена.

Применять это правило можно только после
подтверждения его реализации в 1С.

После успеха:
получить сохранённый документ по id
и показать фактические данные сервера.`,
      },
      {
        title: "Обработать ошибки и пометку удаления",
        text: "Используйте общий API-клиент и обработку сессии из предыдущих уроков.",
        code: `Состояния:
...
400/422 — ошибки шапки или строк;
401     — повторный вход через общий auth flow;
403     — недостаточно прав, без logout;
404     — заявка или товар не найдены;
409     — конфликт операции;
502/504 — сервис временно недоступен.

DELETE /api/requests/{id}:
- только после подтверждения;
- только пометка удаления;
- HTTP 204 не требует чтения JSON;
- после успеха обновить данные.

При потере ответа на сохранение:
результат может быть неизвестен.
Не повторяйте POST автоматически —
сначала проверьте список и данные в 1С.`,
      },
      {
        title: "Проверить результат в 1С",
        text: "Пройдите полный сценарий на учебной базе и сравните форму React с документом в 1С.",
        code: `Проверка:

1. Создать заявку с двумя товарами.
2. Сохранить и записать полученный id и номер.
3. Открыть документ в 1С.
4. Сравнить дату, комментарий и количества.
5. Изменить количество в React.
6. Удалить одну строку и добавить другую.
7. Сохранить и повторно открыть документ.
8. Проверить, что строки не задвоились.
9. Проверить пустую заявку и количество 0.
10. Проверить пометку удаления документа.

Дополнительно:
- удаление средней строки не меняет соседние;
- при ошибке форма сохраняет введённые данные;
- двойное нажатие не отправляет два запроса;
- перезагрузка карточки восстанавливает данные.

Из папки frontend:

npm run build

В SQL FastAPI не создаются таблицы
заявок, товаров или строк документа.`,
      },
      {
        title: "Prompt для Codex или Claude Code",
        text: "Запускайте AI-агента внутри папки frontend, где находится React-проект.",
        content: [
          ...
          {
            type: "snippet",
            label: "Prompt для CRUD-интерфейса заявок",
            body: `Ты работаешь в папке frontend. Реализуй урок 14: CRUD-интерфейс документа «Заявка».

Сначала изучи существующий проект, авторизацию, API-клиент, маршрутизацию и интерфейс товаров из урока 13. Используй готовые компоненты и стиль проекта.

Изменяй только frontend. Документацию в onec и маршруты bridge разрешено читать для проверки контрактов, но не изменять.

Архитектура: React → FastAPI → HTTP-сервис 1С. Документы и строки хранятся только в 1С. FastAPI хранит только users и sessions.
...
PATCH /api/requests/{id};
DELETE /api/requests/{id};
существующие маршруты выбора товаров.

Уточни форматы даты, списка, ответов сохранения и правила изменения items. Не выдумывай контракт. Если правила не определены, укажи блокер и необходимые уточнения.

Особенно важно: подтвердить, заменяет ли переданный items всю табличную часть. Не реализовывай это предположение без подтверждения.
...
Используй существующий ProtectedRoute. Перезагрузка карточки должна загружать документ по id.

Добавь requestsApi.js поверх общего API-клиента:

listRequests;
getRequest;
createRequest;
updateRequest;
markRequestForDeletion.

Сохрани credentials: "include" и существующий механизм CSRF. Не передавай access token 1С в браузер. Не добавляй автоматические повторы изменяющих запросов.

Создай переиспользуемую RequestForm:

номер — значение сервера, без генерации во frontend;
дата — согласно контракту;
комментарий — максимум 200 символов;
табличная часть товаров.

Не добавляй проведение, цены, суммы и согласование.

Создай ReferencePicker для выбора товара:

используй productsApi;
показывай код и название;
сохраняй стабильный product_id;
поддержи загрузку, пустой результат и ошибку;
не разрешай новый выбор помеченного товара;
не удаляй существующие ссылки молча;
поддержи клавиатуру, подписи полей и корректное управление фокусом.

Не добавляй несуществующие параметры поиска. Если API возвращает страницы, не выдавай поиск по загруженной странице за поиск по всему справочнику.
...
Каждой строке назначь стабильный clientRowId при создании или первоначальной загрузке. Не используй индекс массива, product_id или генерируемое при каждом render значение как React key.

Не отправляй clientRowId и отображаемое название товара в API. Обновляй состояние без прямых мутаций. Не объединяй одинаковые товары без требования контракта.
...
выбран product_id;
quantity — конечное число больше нуля;
максимум 3 десятичных знака;
ограничения длины и диапазона — согласно контракту.

Не превращай пустое количество в 0 и не округляй лишнюю точность молча. Показывай ошибки у соответствующей строки.

Сохраняй шапку и строки одним запросом.

Передавай только разрешённые поля. Во время сохранения блокируй повторную отправку и изменение формы. При ошибке не сбрасывай введённые значения.

После успеха загружай сохранённую заявку с сервера. Не показывай успех до подтверждения API. Если обновление после сохранения не удалось, отличай эту ошибку от ошибки самого сохранения.

При таймауте или потере ответа предупреди, что результат операции неизвестен. Не повторяй создание автоматически.

Реализуй подтверждение пометки удаления заявки.

DELETE не должен физически удалять документ. Отмена диалога не отправляет запрос. Поддержи 204 без JSON и обновление списка после успеха.

Обработай loading, empty, error, saving и success.

Для 401 используй существующий flow входа. Для 403 покажи отказ в доступе без logout. Обработай 400/422, 404, 409 и 502/504.

Не позволяй устаревшим ответам перезаписывать другую заявку или пользовательские правки. Предупреждай о потере несохранённых изменений при уходе из формы.

Проверь сборку и существующие тесты. Проверь:

создание с двумя строками;
изменение количества;
удаление средней строки без изменения соседних;
корректный payload без UI-полей;
запрет пустого документа и quantity ≤ 0;
сохранение введённых данных при ошибке;
блокировку двойной отправки;
подтверждение и отмену пометки удаления.

Сквозные проверки выполняй только на учебной базе. Сверь номер, шапку и строки с 1С. Если доступа нет, явно укажи, что интеграционная проверка не выполнена.
...
подтверждённое правило обновления items;
фактически выполненные проверки;
блокеры и непроверенные сценарии.

Не показывай пароли, cookie, токены или ключи.`,
          },
          {
            type: "note",
            text: "Для редактируемых строк нужны стабильные ключи и обновление массива без прямого изменения React state — это предотвращает ошибки при удалении и перестановке строк.",
          },
        ],
      },
    ],
  },
  {
    ...
    subtitle: "Используйте AI-агента для точечных, проверяемых улучшений CRUD-экранов.",
    minutes: "20 мин",
    level: "Средний",
    complete: false,
    need: ["Экраны товаров и заявок из уроков 13–14", "Запущенное React-приложение", "AI coding agent", "Git"],
    goal: "научиться находить проблемы интерфейса с AI, согласовывать небольшие изменения и проверять, что CRUD продолжает работать.",
    steps: [
      {
        title: "Подготовить контекст экрана",
        text: "Покажите агенту конкретный экран, сценарий пользователя и пример текущего стиля. Не ограничивайтесь просьбой «сделай красиво».",
        code: `Передайте агенту:

- маршрут страницы;
- скриншот без секретных данных;
- файлы страницы и компонентов, если известны;
- сценарий: просмотр, создание или редактирование;
- пример другого экрана с нужным стилем;
- конкретную проблему;
...
Экран: /requests/new
Проблема: ошибки строк плохо заметны.
Эталон: форма товара из урока 13.

Сохранить:
- текущие цвета и типографику;
- API-контракт;
- авторизацию;
- правила сохранения документа.`,
      },
      {
        title: "Провести аудит без изменения кода",
        text: "Сначала попросите агента найти проблемы и объяснить их влияние. Скриншот показывает внешний вид, но не доказывает поведение формы.",
        code: `Проверить:

- загрузку, пустой результат и ошибку;
- сохранение и блокировку повторной отправки;
- ошибки рядом с полями и строками;
- сохранность введённых данных после ошибки;
- подтверждение пометки удаления;
- читаемость на узком экране;
- подписи полей и управление клавиатурой;
...
На этом этапе файлы не изменять.`,
      },
      {
        title: "Выбрать небольшой набор правок",
        text: "Согласуйте 1–3 изменения и критерии их приёмки. Одна итерация должна решать понятную задачу.",
        code: `Пример согласованного объёма:

1. Показать ошибку количества под полем.
2. Заблокировать повторное сохранение.
3. Исправить расположение кнопок на узком экране.

Критерии приёмки:

- при quantity = 0 видна ошибка нужной строки;
- повторный клик не отправляет второй запрос;
- кнопки доступны при ширине 375 px;
- введённые значения не теряются;
- десктопная форма остаётся рабочей.
...
- переписывание авторизации.`,
      ...
      {
        title: "Внести изменения в отдельной ветке",
        text: "Перед работой проверьте текущие изменения. Агент должен сохранить чужую работу и ограничиться согласованными файлами.",
        content: [
          { type: "command", label: "Проверить текущее состояние", code: "git status --short" },
          { type: "command", label: "Создать ветку правок", code: "git switch -c feat/crud-ui-polish" },
          {
            type: "note",
            text: "Правила работы: изменять только согласованный UI; переиспользовать существующие компоненты; не добавлять глобальные стили без необходимости; не менять bridge и onec; не менять JSON-поля и маршруты; не отключать проверки или CSRF; не выполнять массовый рефакторинг. Если нужна правка вне согласованного объёма — сначала объяснить причину и запросить решение.",
          },
        ],
      },
      {
        title: "Проверить интерфейс и CRUD",
        text: "Сборка проверяет проект, но не заменяет проверку поведения в браузере.",
        content: [
          { type: "command", label: "Собрать frontend", code: "npm run build" },
          {
            type: "steps",
            items: [
              "Открыть изменённый экран.",
              "Проверить обычный и узкий экран.",
              "Пройти форму клавиатурой.",
              "Вызвать ошибку заполнения.",
              "Проверить успешное сохранение.",
              "Проверить ошибку API без потери данных.",
              "Проверить отмену пометки удаления.",
              "Проверить повторное открытие сохранённого объекта.",
            ],
          },
          {
            type: "note",
            text: "Для заявки: добавить и удалить строку, убедиться, что соседние строки не изменились. Изменять данные только в учебной базе. Не выдавать mock-проверку за проверку с 1С.",
          },
        ],
      },
      {
        title: "Проверить diff и зафиксировать результат",
        text: "Оцените не только внешний вид, но и объём изменений. В коммит должны попасть только правки этой задачи.",
        content: [
          { type: "command", label: "Посмотреть статистику diff", code: "git diff --stat" },
          { type: "command", label: "Посмотреть diff frontend", code: "git diff -- frontend" },
          { type: "command", label: "Проверить статус", code: "git status --short" },
          { type: "command", label: "Добавить выбранные изменения", code: "git add -p -- frontend" },
          { type: "command", label: "Проверить подготовленные изменения", code: "git diff --cached" },
          { type: "command", label: "Создать коммит", code: `git commit -m "Improve CRUD form states and layout"` },
          {
            type: "note",
            text: "Итог агента должен включать: что исправлено; какие файлы изменены; что проверено; что не удалось проверить; скриншоты до/после, если доступны.",
          },
        ],
      },
      {
        title: "Prompt 1 — проверка без изменений",
        text: "Запускайте AI-агента внутри папки frontend. На этом шаге агент только анализирует код и предлагает правки, но не редактирует файлы.",
        content: [
          { type: "command", label: "Перейти в папку frontend", code: "cd frontend" },
          { type: "command", label: "Запустить Codex", code: "codex" },
          { type: "command", label: "Или запустить Claude Code", code: "claude" },
          {
            type: "snippet",
            label: "Prompt для аудита экрана",
            body: `Проверь CRUD-экран React. На этом этапе не изменяй файлы.

Экран: [маршрут страницы]
Сценарий: [что делает пользователь]
Проблема: [что неудобно]
Эталон стиля: [страница или скриншот]

Изучи компоненты экрана, API-слой и используемые стили. Если доступен браузер, проверь поведение. Если нет — явно отдели выводы по коду от предположений по скриншоту.

Проверь:

loading, empty, error и success;
сохранение, двойной клик и ошибки валидации;
сохранность введённых данных после ошибки;
подтверждение пометки удаления;
мобильное отображение;
подписи полей, фокус и управление клавиатурой;
соответствие текущему стилю.

Учти архитектуру курса:

React обращается только к FastAPI;
авторизация работает через HttpOnly cookie;
товары и заявки хранятся в 1С;
DELETE означает пометку удаления.

Верни не больше пяти приоритетных проблем. Для каждой укажи место, доказательство, влияние, минимальную правку и способ проверки.

Предложи 1–3 правки для первой итерации. Не выполняй их до согласования. Не изменяй данные в 1С для аудита без отдельного разрешения.`,
          },
        ],
      },
      {
        title: "Prompt 2 — согласованные правки",
        text: "Запускайте этот prompt только после согласования правок из первого prompt, в отдельной ветке.",
        content: [
          {
            type: "snippet",
            label: "Prompt для внесения согласованных правок",
            body: `Внеси только согласованные изменения CRUD-интерфейса.
...
Сначала проверь текущую ветку и git status. Сохрани существующие изменения пользователя.

Изменяй только frontend и только то, что необходимо для этой задачи. Используй текущие компоненты, цвета, отступы и типографику.

Не переписывай экран целиком. Не меняй API-контракты, авторизацию, bridge, документацию onec или бизнес-правила. Не добавляй UI-библиотеки и не отключай защитные проверки.

Если изменение требует расширения объёма работ, остановись и объясни причину.

После правок:

выполни сборку и доступные проверки проекта;
проверь согласованные сценарии в браузере, если он доступен;
проверь обычную и узкую ширину экрана;
убедись, что не появились повторные запросы или потеря данных формы;
просмотри diff на посторонние изменения.

Операции записи проверяй только на явно разрешённой учебной базе. Если она недоступна, укажи, что сквозной CRUD не проверен.

Верни изменённые файлы, результат по каждому критерию приёмки и непроверенные сценарии. Приложи скриншоты до/после, если они действительно получены.

Не создавай коммит и не публикуй изменения автоматически.`,
          },
          {
            type: "note",
            text: "AI не меняет код на этапе аудита (Prompt 1) и не затрагивает API-контракты ради улучшения UI ни на одном из этапов.",
          },
        ],
      },
    ],
